...
waittime="120"   # number of seconds between executions of loop
...
do
	...
	if [[ X"$lasttitle" != X"$curtitle" || $was_awaken = true ]]; then
		perform_write=true
		timew start misc
		if echo "$curtitle" | grep "\bNetflix\b\|\bnetflix\b" 2>/dev/null >/dev/null;
		then
			timew tag @1 netflix entertainment
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "Hotstar" 2>/dev/null >/dev/null;
		then
			timew tag @1 hotstar entertainment
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "freading" 2>/dev/null >/dev/null;
		then
			timew tag @1 reading fiction
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "nfreading" 2>/dev/null >/dev/null;
		then
			timew tag @1 reading nonfiction
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "Visual Studio" 2>/dev/null >/dev/null;
		then
			timew tag @1 vscode project
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "rkosh" 2>/dev/null >/dev/null;
		then
			timew tag @1 project rkosh
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "rdnss" 2>/dev/null >/dev/null;
		then
			timew tag @1 project rdnss
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "yasdb" 2>/dev/null >/dev/null;
		then
			timew tag @1 project yasdb
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "sakgames" 2>/dev/null >/dev/null;
		then
			timew tag @1 sakgames project
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "Wikipedia\|wikipedia" 2>/dev/null >/dev/null;
		then
			timew tag @1 wikipedia
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "Algorithms\|Networking\|Compilers\|Interpreters\|Operating\|Design" 2>/dev/null >/dev/null;
		then
			timew tag @1 studying computer science
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "Tree\|tree\|Graph\|graph\|Operating\|Design" 2>/dev/null >/dev/null;
		then
			timew tag @1 studying computer science
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "Rust\|rust\|rustbook" 2>/dev/null >/dev/null;
		then
			timew tag @1 language rust
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "Python\|python" 2>/dev/null >/dev/null;
		then
			timew tag @1 language python
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "\bgolang\b\|\bGo\b\|\bgo\b\|\bGolang\b" 2>/dev/null >/dev/null;
		then
			timew tag @1 language go lang golang
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "Javasript\|JS|\JavaScript\|jsbook" 2>/dev/null >/dev/null;
		then
			timew tag @1 language javasript
			tagged=true
			timew untag @1 misc
		fi
		if echo "$curtitle" | grep "Cpp\|cpp\|CPP\|c++\|C++" 2>/dev/null >/dev/null;
		then
			timew tag @1 language cpp
			timew untag @1 misc
			tagged=true
		fi
		#if echo "$curtitle" | grep "C\|c" 2>/dev/null >/dev/null;
		#then
		#	timew tag @1 language c
		#	timew untag @1 misc
		#	tagged=true
		#fi
		if echo "$curtitle" | grep "CareerIssueList\|cil" 2>/dev/null >/dev/null;
		then
			timew tag @1 metawork cil
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "gtask" 2>/dev/null >/dev/null;
		then
			timew tag @1 metawork cil
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "timew" 2>/dev/null >/dev/null;
		then
			timew tag @1 metawork timewarrior
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "taskw" 2>/dev/null >/dev/null;
		then
			timew tag @1 metawork taskwarrior
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "uLogMe" 2>/dev/null >/dev/null;
		then
			timew tag @1 metawork ulogme
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "What did I do?" 2>/dev/null >/dev/null;
		then
			timew tag @1 metawork journaling
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "Trilium" 2>/dev/null >/dev/null;
		then
			timew tag @1 metawork notes
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "Monitor\|systemtop" 2>/dev/null >/dev/null;
		then
			timew tag @1 metawork monitoring system
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "bashrc" 2>/dev/null >/dev/null;
		then
			timew tag @1 metawork bashrc
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "nano" 2>/dev/null >/dev/null;
		then
			timew tag @1 metawork nano
			timew untag @1 misc
			tagged=true
		fi
		if echo "$curtitle" | grep "gedit" 2>/dev/null >/dev/null;
		then
			timew tag @1 metawork gedit
			timew untag @1 misc
			tagged=true
		fi
		...
		if echo "$curtitle" | grep "CloseTracking" 2>/dev/null >/dev/null;
		then
			tagged=true
			timew untag @1 misc
			timew delete @1
		...
		if [ $tagged = false ];
		then
			if echo "$curtitle" | grep "Opera\|Firefox\|Chrome\|Chromium" 2>/dev/null >/dev/null;
			then
			#timew delete @1
			#timew start browsing
			timew tag @1 browsing
			timew untag @1 misc
			tagged=true
			fi
			#if tagged==false ;
